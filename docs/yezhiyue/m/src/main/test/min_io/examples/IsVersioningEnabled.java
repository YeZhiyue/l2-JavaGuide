...
public class IsVersioningEnabled {
  ...
  public static void main(String[] args)
    ...
    try {
      ...
      if (!minioClient.bucketExists(BucketExistsArgs.builder().bucket(Config.BUCKETS[0]).build())) {
        minioClient.makeBucket(MakeBucketArgs.builder().bucket(Config.BUCKETS[0]).build());
        System.out.println("my-bucketname is created successfully");
      }

      boolean isVersioningEnabled =
          minioClient.isVersioningEnabled(
              IsVersioningEnabledArgs.builder().bucket(Config.BUCKETS[0]).build());
      ...
      minioClient.enableVersioning(EnableVersioningArgs.builder().bucket(Config.BUCKETS[0]).build());
